    ...
    class Better_Messages_HivePress
    {
        ...
        public function render_small_listing_button(){
            ...
            if( ! is_user_logged_in() && ! Better_Messages()->guests->guest_access_enabled() ){
                ...
                $subject = sprintf(esc_html_x('Question about listing "%s"', 'Dokan Integration (Listing page)', 'bp-better-messages'), get_the_title());
                ...
                if( $post_type === 'hp_booking' ){
                    ...
                    if( $listing ) {
                        $subject = sprintf(esc_html_x('Question about booking "%s" of "%s"', 'Dokan Integration (Booking item)', 'bp-better-messages'), get_the_title($booking_id), get_the_title( $listing ));
        ...
        public function render_big_listing_button(){
            ...
            if( ! is_user_logged_in() && ! Better_Messages()->guests->guest_access_enabled() ){
                ...
                $current_user_id = Better_Messages()->functions->get_current_user_id();
                $user_id = get_the_author_meta('ID');
                $subject = sprintf(esc_html_x('Question about your listing "%s"', 'Dokan Integration (Product page)', 'bp-better-messages'), get_the_title());
